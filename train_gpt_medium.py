        ...
        for group in self.param_groups:
            ...
            for base in range(len(params)):
                ...
                if not state:
                    ...
                    state["smoothed_update_buffer"] = torch.zeros_like(p_slice)
                exp_avg = state['exp_avg']
                exp_avg_sq = state['exp_avg_sq']
                smoothed_update_buffer = state["smoothed_update_buffer"]
                state['step'] += 1
                t = state['step']
                # weight decay
                if wd != 0:
                ...
                smoothed_update_buffer.mul_(update_smoothing).add_(update, alpha=1.0-update_smoothing)
                p_slice.add_(other=smoothed_update, alpha=-1.0)
                idx += 1
...
@torch.compile
def update(acc_bf16_view_u16: Tensor, mantissa: Tensor, momentum_buffer: Tensor, smoothed_update_buffer: Tensor, grad: Tensor, momentum: Tensor, update_smoothing: Tensor, extra_update: Tensor, eff_lr: Tensor, eff_weight_decay: Tensor):
    ...
    v = (1-extra_update) * smoothed_update_buffer + extra_update * v
...
class Muon(torch.optim.Optimizer):
    ...
    """
    def __init__(self, params, lr=0.02, weight_decay=0.01, momentum=0.95, update_smoothing=0.1, extra_update=0.0, rank=0, world_size=1):
        self.rank = rank
        self.world_size = world_size
        defaults = dict(lr=lr, weight_decay=weight_decay, momentum=momentum, update_smoothing=update_smoothing, extra_update=extra_update)
    ...
    def step(self):
        ...
        for group in self.param_groups:
            ...
            extra_update = torch._as_tensor_fullprec(group["extra_update"])
            for base_i in range(len(params))[::self.world_size]:
                if base_i + self.rank < len(params):
                    ...
                    update(
                        ...
                        p.grad, momentum, update_smoothing, extra_update,
